  ...
  for g, group in enumerate(group_choices):
    for cdid in users_list[g]:
      for ctype in ['Genuine', 'Impostor']:
        cid = ctype + '_%d' % cdid
        if verbose>1: print("  Adding user '%s' on '%s' group..." % (cid,group))
        session.add(Client(cid, group, ctype))
...
def add_files(session, imagedir, verbose):
  ...
  def add_file(session, basename, userdir, sessiondir, userid, shotid):
    """Parse a single filename and add it to the list."""
    shotid = int(basename[-1])
    session.add(File(userid, os.path.join(userdir, sessiondir, basename), int(sessiondir[-1]), shotid))
  ...
  for userdir in os.listdir(imagedir):
    ...
    for sessiondir in sessiondirs:
      ...
      for filename in filenames:
        ...
        if extension == '.svc' and 'sg' in basename:
          if verbose>1: print("  Adding file '%s'..." % (basename))
          shotid = int(basename[-1])
          if shotid in [1, 2]:
            userid = 'Genuine_' + '%d' % int(userdir[-4:])
          elif shotid in [6, 7]:
            userid = 'Genuine_' + '%d' % int(userdir[-4:])
            shotid = shotid - 3
          elif shotid in [3, 4, 5]:
            userid = 'Impostor_' + '%d' % int(userdir[-4:])
            shotid = shotid - 2
          add_file(session, basename, userdir, sessiondir, userid, shotid)
  ...
  for proto in protocols:
    ...
    for key in range(len(protocolPurpose_list)):
      ...
      if(key == 0): # world
        ...
        q = session.query(File).join(Client).filter(and_(Client.sgroup == 'clientDev', Client.stype == 'Genuine')).filter(File.session_id.in_(enroll_session))
        for k in q:
          if verbose>1: print("    Adding protocol file '%s'..." % (k.path))
          pu.files.append(k)

      elif(key == 2): #dev probe
        q = session.query(File).join(Client).filter(and_(Client.sgroup == 'clientDev', Client.stype == 'Genuine')).filter(File.session_id.in_(client_probe_session))
        for k in q:
          if verbose>1: print("    Adding protocol file '%s'..." % (k.path))
          pu.files.append(k)
        q = session.query(File).join(Client).filter(and_(Client.sgroup == 'impostorDev', Client.stype == 'Genuine'))
        for k in q:
          #if verbose>1: print("    Adding protocol file '%s'..." % (k.path))
          pu.files.append(k)
        q = session.query(File).join(Client).filter(and_(Client.sgroup == 'clientDev', Client.stype == 'Impostor'))
        for k in q:
          if verbose>1: print("    Adding protocol file '%s'..." % (k.path))
          pu.files.append(k)

      elif(key == 3): #test enrol
        q = session.query(File).join(Client).filter(and_(Client.sgroup == 'clientEval', Client.stype == 'Genuine')).filter(File.session_id.in_(enroll_session))
        for k in q:
          if verbose>1: print("    Adding protocol file '%s'..." % (k.path))
          pu.files.append(k)

      elif(key == 4): #test probe
        q = session.query(File).join(Client).filter(and_(Client.sgroup == 'clientEval', Client.stype == 'Genuine')).filter(File.session_id.in_(client_probe_session))
        for k in q:
          if verbose>1: print("    Adding protocol file '%s'..." % (k.path))
          pu.files.append(k)
        q = session.query(File).join(Client).filter(and_(Client.sgroup == 'impostorEval', Client.stype == 'Genuine'))
        for k in q:
          if verbose>1: print("    Adding protocol file '%s'..." % (k.path))
          pu.files.append(k)
        q = session.query(File).join(Client).filter(and_(Client.sgroup == 'clientEval', Client.stype == 'Impostor'))
        for k in q:
          if verbose>1: print("    Adding protocol file '%s'..." % (k.path))
          pu.files.append(k)
