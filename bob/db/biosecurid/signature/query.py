...
class Database(bob.db.verification.utils.SQLiteDatabase,bob.db.verification.utils.Database):
  ...
  def __init__(self, original_directory = None, original_extension = '.svc'):
  ...
  def clients(self, protocol=None, groups=None, types=None):
    ...
    types
      The types to which the clients belong either from ('genuine', 'impostor')
      If no groups are specified, then both clients are impostors are listed.
    ...
    types = self.check_parameters_for_validity(types, "types", self.client_types())
    # List of the clients
    q = self.query(Client)
    if groups and types:
      q = q.filter(and_(Client.sgroup.in_(groups), Client.stype.in_(types)))
    elif groups:
      q = q.filter(Client.sgroup.in_(groups))
    elif types:
      q = q.filter(Client.stype.in_(types))
  ...
  def models(self, protocol=None, groups=None):
    ...
    """
    types = 'Genuine'
    types = self.check_parameters_for_validity(types, "types", ('Genuine',))
    # List of the clients
    q = self.query(Client)
    if groups:
      q = q.filter(and_(Client.sgroup.in_(self.__group_replace_alias__(groups)), Client.stype.in_(types)))
    else:
      q = q.filter(and_(Client.sgroup.in_(['clientDev','clientEval']), Client.stype.in_(types)))
    q = q.order_by(Client.id)
    return list(q)
